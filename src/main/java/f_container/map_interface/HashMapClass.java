...
/**
 * HashMap源码分析
 * HashMap(线程不安全)<===>Hashtable中方法大多被synchronized关键字修饰(线程安全)
 * HashMap存储的key、value都可以为null<===>Hashtable存储的key、value都不能为null
 */
public class HashMapClass<K, V> {
    ...
    static final class TreeNode<K, V> extends Entry<K, V> {
         ...
         * this node.
         */
        final Node<K, V> untreeify(HashMapClass<K, V> map) {
         ...
         */
        final TreeNode<K, V> putTreeVal(HashMapClass<K, V> map, Node<K, V>[] tab,
                                        int h, K k, V v) {
            Class<?> kc = null;
         ...
         */
        final void removeTreeNode(HashMapClass<K, V> map, Node<K, V>[] tab,
                                  boolean movable) {
            int n;
         ...
         */
        final void split(HashMapClass<K, V> map, Node<K, V>[] tab, int index, int bit) {
            TreeNode<K, V> b = this;
